...
function Section() {
  ...
  const handleDeleteSection = async (id) => {
    const isConfirmed = confirm(
      'Deleting this section will also delete content under this section'
    );
    if (isConfirmed) {
  ...
  const handleEditSection = async () => {
    ...
    try {
      const response = await apiClient.put(`/api/section/${editSectionId}`, {
        name: sectionName,
      });
      if (response.status === 200) {
        fetchSections();
          ...
          <Card>
            <Card.Body>
              ...
              <Button
                ...
                style={{ fontSize: '15px', backgroundColor: '#1861bf' }}
              >
                <FontAwesomeIcon
                  icon={faCheckCircle}
                    ...
                    {currentSections.map((section) => (
                      <tr key={section.sectionId}>
                        ...
                        <td>
                          <Dropdown>
                            <Dropdown.Toggle
                              name="sectionActionBtn"
                              id="sectionActionBtnId"
                              style={{
                                height: '30px',
              ...
              <Row>
                <Col xs md={8} style={{ fontSize: '17px' }}>
                  <p style={{ marginLeft: '20px' }}>
                    ...
                    {Math.min(indexOfLastSection, sections.length)} of{' '}
