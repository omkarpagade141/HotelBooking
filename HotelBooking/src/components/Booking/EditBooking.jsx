...
function EditBooking() {
  ...
  const handleAddItem = async () => {
    ...
    const response = await apiClient.post(
      `http://localhost:8080/api/items/booking/${BookingId}`,
      {
        itemList: [
          {
    ...
    if (confirmed) {
      try {
        const response = await apiClient.delete(
          `http://localhost:8080/api/items/booking/${BookingId}/item/${itemId}`
        );
  ...
  return (
    <div>
      <Card>
        <Card.Body>
          <h3>
            Edit Booking ({bookingData ? bookingData.customer.fullName : ''}){' '}
          </h3>
          <hr />
          <Row>
            <Col xs={6} md={2}>
